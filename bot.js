...
async function tweetEvent(tweet) {
    ...
    if(tweetText.includes('@covidtwtbot')) {
        if(tweet.in_reply_to_screen_name != 'covidtwtbot' && tweet.in_reply_to_screen_name != null) {
            console.log("cp1")
            ...
            if(!originalTweetText.includes('needed')) {
                if(!originalTweetText.includes('need')) {
                    if(!originalTweetText.includes('require')) {
                        moveAhead = false
                    }
                }
            }
